...
export const PI_TODO_SYSTEM_PREFIX = "[system:pi-todo]";
export const PI_TODO_CONVERSATION_EVENT = "pi-todo:conversation";
...
export interface TodoConversationEvent {
	...
	timestamp: number;
	conversation: {
		prefix: typeof PI_TODO_SYSTEM_PREFIX;
		kind: "user_message";
...
function prefixText(text: string): string {
	return text.startsWith(PI_TODO_SYSTEM_PREFIX) ? text : `${PI_TODO_SYSTEM_PREFIX}\n${text}`;
}
...
function prefixContent(content: string | (TextContent | ImageContent)[]): string | (TextContent | ImageContent)[] {
	...
	if (firstTextIndex === -1) {
		return [{ type: "text", text: PI_TODO_SYSTEM_PREFIX }, ...content];
	}
...
function emitTodoConversationEvent(pi: ExtensionAPI, event: TodoConversationEvent): void {
	pi.events.emit(PI_TODO_CONVERSATION_EVENT, event);
}
