...
import { isTodoWriteDetails } from "../guards";
import { getTodoResultLines, type TodoItem, type TodoWriteDetails } from "../state";
...
export function registerTodoReadTool(pi: ExtensionAPI, getCurrentTodos: () => TodoItem[]): void {
	pi.registerTool({
		...
		promptGuidelines: [
			"Use todoread when you need the current todo list before deciding how to update it.",
			"todoread returns the latest session todo list managed by todowrite.",
		],
